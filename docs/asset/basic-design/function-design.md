...
|---|---|
| 文書ID | BD-03 |
| 版数 | V1.3（2026-09-05・P0-4 Step 5 回写：F-21/F-23/F-24 実装済＋§4.9-1/§5.9-1/§6.9-1・2/§7.9-1/§12-3 未決销账＋MSG 文面確定。前版 V1.2＝2026-09-04 D-3 回写：F-20/F-24/F-25/F-26 状態列更新） |
| 作成日 | 2026-08-31 |
...
- 状態表記：✅ 既存実装・実装済｜🔵 P0 計画（未実装）｜⚪ P1。**本書の対象機能（F-20〜F-26）は全て実装済**（F-20：SF 側受入口 6b9d970＋Booking 側送信 `ProjectionSenderService` 3d88923・F-24：SF 側 Apex 受付 S-5〔`BookingSiteController`〕＋P0-4 LWC 画面〔`bookingProjectionList`・2026-09-05〕・F-25：S-4〔`BookingCommandQueueable`〕＋Booking 側 B-2/B-3 8581f50・F-26：S-4 バージョンゲート付き書き戻し・F-21：P0-4〔frontend `SalesforceWorkbenchEntry`・2026-09-05〕・F-23：P0-4〔LWC `bookingProjectionList`・2026-09-05〕）。**P0-4 実機検証済（2026-09-05・MV-01/02/03 後半/07/08 全件 PASS・CHK-03 参照）**。
...
## 3. F-20：予約投影（✅ 実装済・2026-09-04：SF 側受入口〔6b9d970〕＋Booking 側送信〔`ProjectionSenderService`・3d88923〕）
...
| 2 | 【決定済 2026-09-02】同期呼出（待つ）・タイムアウト 3000ms・失敗時は正本応答を成功のまま syncStatus=ERROR 記録（IF-01 §3 の短時間待ち前提と整合・CHK-02 C-4） | 決定済み（2026-09-02） |

## 4. F-21：管理者入口遷移（✅ 実装済・2026-09-05：P0-4〔frontend `SalesforceWorkbenchEntry` 三態〕＋MV-02 実機検証済）
...
| 出力 | 入口ボタンの表示状態（有効/無効/非表示）と Site URL への遷移 | BD-04 §2.1 S-04（入口ボタン＝F-21 実装済・2026-09-05） |
...
- 非 ADMIN・マッピング不存在/inactive：**非 ADMIN は非表示・ADMIN＋inactive は無効表示**（エラーメッセージは出さず操作不能＝拍板 7 Option 2・2026-09-05 実装済）。メッセージID を付ける場合は BD-05 の MSG-xx 体系に従う。
...
| No. | 未決事項 | 決定期限 |
|---|---|---|
| 1 | 【決定済 2026-09-05・拍板 7 Option 2】非 ADMIN＝非表示・ADMIN＋inactive＝無効表示（エラーメッセージなし・F-21 §4.5 実装済） | 決定済み（2026-09-05） |
| 2 | 【決定済 2026-09-02】Prisma モデル `StaticOperatorMapping`（`static_operator_mappings`・CHK-02 C-3） | 決定済み（2026-09-02） |

## 5. F-22：Site 独立ログイン（既存・✅ 検証済〔ログイン部分＝MV-03＋閲覧範囲限定＝MV-07・実機検証済 2026-09-05〕）
...
| No. | 未決事項 | 決定期限 |
|---|---|---|
| 1 | 【実施済 2026-09-05】「権限付与済みページのみ閲覧可能」の検証（MV-07）＝P0-4 実機検証完了（行級＝他 Account ExternalId 直指定 submitCancel 拒否 MSG-03・字段級＝getProjections 9 項目最小集〔DevTools 実測〕・CHK-03 参照） | 実施済み（2026-09-05） |

## 6. F-23：投影リスト表示（✅ 実装済・2026-09-05：chogeer LWC `bookingProjectionList`〔TERM-33〕＋MV-07/MV-08 実機検証済）
...
| 分岐 | 処理 | メッセージ方針（設計値・実装確定＝BD-05 §3.1） |
|---|---|---|
...
- 現サイトのサンプルテンプレートは P0-4 で独自 LWC（`bookingProjectionList`）に置換済み（2026-09-05・ホームページ＝同 LWC・ログイン限定）。

### 6.9 未決事項

| No. | 未決事項 | 決定期限 |
|---|---|---|
| 1 | 【決定済 2026-09-03・拍板 1】LWC バンドル名＝`bookingProjectionList`（TERM-33・2026-09-05 実装済。BD-04 §5 と同源） | 決定済み（2026-09-03／実装済 2026-09-05） |
| 2 | 【決定済 2026-09-03・拍板 4】取消可否判定の表示条件＝`Status__c` PENDING/CONFIRMED 活性・CANCELLED/COMPLETED 非活性＋MSG-04（tooltip 形態・2026-09-05 実装済） | 決定済み（2026-09-03／実装済 2026-09-05） |

## 7. F-24：キャンセルコマンド受理（✅ 実装済・2026-09-04/09-05：P0-3 Apex 側 S-5〔`BookingSiteController`〕＋P0-4 LWC 画面〔`bookingProjectionList`〕＋MV-08 実機検証済）
...
| No. | 未決事項 | 決定期限 |
|---|---|---|
| 1 | 【決定済 2026-09-03・拍板 3】ポーリング間隔＝3 秒・最長 60 秒（20 tick）自動停止＋手動「再読み込み」。Apex 側は非 cacheable `pollCommandStatus`（拍板 2b・クライアントキャッシュ回避）＋LWC 側重入ガード（`pollInFlight`） | 決定済み（2026-09-03／実装済 2026-09-05） |
| 2 | 【決定済 2026-09-01】`resultCode` 値域＝CD-12（7 値封闭集・BD-09 §5 未決 4 と同日クローズ） | 決定済み（2026-09-01） |

## 8. F-25：コマンドバックグラウンド実行（✅ 実装済・2026-09-03/09-04：SF 側 S-4〔`BookingCommandQueueable`〕＋Booking 側 B-2/B-3〔8581f50〕）
...
## 9. F-26：結果書き戻し（✅ 実装済・2026-09-03/09-04：S-4 バージョンゲート付き書き戻し実装済）
...
| 2 | 【決定済 2026-09-02】Prisma モデル `StaticOperatorMapping`（§4.9 と同源・CHK-02 C-3） | 決定済み（2026-09-02） |
| 3 | 【決定済 2026-09-05・拍板 6】各機能のメッセージ文面＝BD-05 §3.1 設計値 6 件（MSG-01〜06）で確定（Apex 暫定文 2 常量も設計値へ対齐済・MSG-04 は tooltip 形態） | 決定済み（2026-09-05） |
