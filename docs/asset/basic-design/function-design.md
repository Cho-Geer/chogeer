...
- 状態表記：✅ 既存実装｜🔵 P0 計画（未実装）｜⚪ P1。**本書の対象機能は特記（F-22 と F-20）のない限りすべて 🔵 計画・未着手であり、記載値は設計値（未実装）である。F-20 は SF 側受入口のみ 2026-09-02 に実施済（6b9d970・Booking 側送信は P0-3 計画）。**
...
## 3. F-20：予約投影（🔵 P0-3 計画・部分実施：SF 側受入口済〔6b9d970〕・Booking 側残）
