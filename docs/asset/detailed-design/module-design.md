...
| 例外処理 | 検証 NG 時に **401（`AuthenticationException`）**・**env `INTEGRATION_TOKEN` 未設定時は 500 fail-closed（実装：integration.guard.ts:40-50）**を throw（実在クラス・`src/common/exceptions/business.exceptions.ts`）。403（`AuthorizationException`）は Guard 層ではなく **Service 層 RULE-12**（静的マッピング不存在/inactive・非 ADMIN/ACTIVE・§2.3）で throw される。業務状態遷移・コマンド状態は一切変更しない（RULE-14 注記・MV-11）。CSRF ミドルウェアとの干渉は BD-11 未決事項 3 として確認対象 → 【解決 2026-09-02】干渉なし（CSRF middleware の Bearer 豁免が適用・CHK-02 C-9） |
...
| 処理概要 | 1) 正本変更トランザクション確定後に呼出（トランザクション内で version+1・syncStatus=PENDING 済・RULE-08）。2) eventId・correlationId を採番（CF-05・UUID・設計値）。3) ホワイトリストのみからペイロード生成。4) OAuth 2.0 JWT Bearer（外部クライアントアプリケーション ECA・A2・TERM-20）で `POST /services/apexrest/integrations/bookings/projection` を呼出。5) 応答判定：受理（初回・より新しい version）→ `syncStatus=SYNCED` 更新。拒否・認証系・一時的障害 → `syncStatus=ERROR` 記録（BD-09 §3.8）。6) 手動 Retry（同一 eventId）は BIZ-15 手順に依拠（Retry UI は P1 保留）※投影の原 eventId 再送は実装偏差により不可〔eventId 非永続・projection-sender.service.ts:87〕・代替＝正本再保存で新 eventId〔手順書第4章・拍板 2026-09-04①〕 |
| 呼び出し関係 | 呼出元：既存の正本変更処理（bookings モジュールの作成/変更/取消・F-06〜F-08）と IntegrationCommandsService（§2.3・コマンド取消）のトランザクション確定後。呼出先：Apex REST（BookingProjectionRest・§3.1）。OAuth クライアント依存は P0-3 で新規導入（現状 package.json に HTTP クライアント・OAuth 依存なし＝実測）※追記 2026-09-04：3d88923 で @nestjs/axios 3.1.3 導入済み |
...
| 使用 SQL | SOQL 要点（設計値）：`SELECT Id, CurrentVersion__c, LastEventId__c, SyncStatus__c, CorrelationId__c, LastError__c FROM Booking__c WHERE BookingExternalId__c = :extId FOR UPDATE`＋~~`WITH SECURITY_ENFORCED`~~→**除去（2026-09-03・S-4 登記②解消）**：F-26 書き戻し（§3.2 再利用）が Site ユーザー enqueue 経路で成立するよう、本 FOR UPDATE を**内部システム操作＝system context と明示**（with sharing 行級は維持・A2 統合ユーザー経路の挙動不変。ユーザー拍板 Option 1・ADR-3 同日）。当初の `SECURITY_ENFORCED`（ShowcaseIntegrationRest の `queryUsersByIds` 実測パターン流用）は A2 経路前提の防御であり、Site 経路では監査 3 項目（LastEventId/CorrelationId/LastError＝F-32 表 1 Read ×）の FLS 違反で写回が必然失敗するため除去。**さらに Upsert も同様（実測）：`with sharing` クラス内の `Database.upsert`（update 経路）はユーザー権限で強制され・`AccessLevel.SYSTEM_MODE` 指定でも回避不能（org 実証）→ `BookingProjectionDmlHelper`（`without sharing`・投影入口専用の upsert 委譲 1 メソッド）へ委譲**（呼出元制限＝BookingProjectionRest のみ・クラスヘッダ注記）。Upsert は部分成功許容 |
