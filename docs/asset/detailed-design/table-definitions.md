...
|---|---|
| 文書ID | DD-01 |
| 版数 | V1.1（2026-09-03・§4 SF 側オブジェクト作成済反映〔P0-2 S-1〜S-4〕） |
| 作成日 | 2026-08-31 |
...
- 事実源（実測）：`booking-backend/prisma/schema.prisma`（2026-08-31 読取・13 モデル全フィールド）。Salesforce 側オブジェクトは BD-07 §3 の計画項目（🔵 契約凍結済み（2026-09-01））であり、本書の SF 側記載は設計値（P0-2 契約）に基づく。**両オブジェクトは 2026-09-02 作成済**〔CHK-01 S-1〜S-4・aec1bf4/f020618/bc7a4ac〕（§4 参照）。
- 状態表記：✅＝既存実装（schema.prisma 実測）｜🔵＝P0 計画。本書の Booking 側 13 テーブルは ✅、§2.15 の integration_commands は schema 実装済（2026-09-02・booking-backend 71c88c8・runtime 接続は P0-3 残）、Salesforce 側 2 オブジェクトは作成済（§4・2026-09-02）。
...
## 4. Salesforce 側オブジェクト（🔵 設計値・P0-2 契約・作成済 2026-09-02）

Salesforce 側の 2 オブジェクトは SF プラットフォームが DDL を管理するため、テーブル定義書としての DDL は存在しない。ここでは BD-07 §3 の計画項目（項目一覧・物理名候補）を設計値として記録する。**記載は設計値（P0-2 契約）だが、実オブジェクトは 2026-09-02 に作成済**（CHK-01 S-1〔Booking__c 13 項目〕・S-2〔Booking_Command__c 16 項目〕・S-3〔OWD〕・S-4〔権限〕・aec1bf4/f020618/bc7a4ac）。型は Salesforce 標準データ型の計画値（2026-09-03 実測照合・1:1 一致）。

### 4.1 Booking__c（予約投影・TERM-09）

- 状態：🔵 契約凍結済み（2026-09-01）・作成済（2026-09-02・CHK-01 S-1/S-3/S-4）。External OWD=Private＋Sharing Set（Account 隔離・TERM-32）前提（設定済）。
...
- 状態：🔵 契約凍結済み（2026-09-01）・作成済（2026-09-02・CHK-01 S-2/S-3/S-4）。External OWD=Private＋Sharing Set（Account 隔離）前提（設定済）。
